...
export type ISC = ISC1 | ISC2;

export interface ISC1 {
  type: 1;
  version: string;
  name: string;
  bones: ISCBone[];
...
export const ISC = {
  ...
  load(buf: Buffer): ISC {
    ...
    switch (version) {
      case "RT04":
        return load2(buf, version);
      default:
        return load1(buf, version);
    }
  },
};

function load1(buf: Buffer, version: string): ISC1 {
  ...
  for (let i = 0; i < numBones; i++) {
    ...
    if (type !== 0x454e4f42) throw new Error(`unknown bone type: ${type}`);
  ...
  for (let i = 0; i < numSlots; i++) {
    ...
    if (type !== 0x544f4c53) throw new Error(`unknown slot type: ${type}`);
  ...
  for (let i = 0; i < numForms; i++) {
    ...
    if (type !== 0x4d524f46) throw new Error(`unknown form type: ${type}`);
  ...
  for (let i = 0; i < numSkins; i++) {
    ...
    if (type !== 0x4e494b53) throw new Error(`unknown skin type: ${type}`);
  ...
  for (let i = 0; i < numMeshs; i++) {
    ...
    if (type !== 0x4853454d) throw new Error(`unknown mesh type: ${type}`);
    ...
    function readVertex() {
      ...
      if (isSpring) {
        ...
        vertex = {
          src: {
            x: buf.readFloatLE(offset + 8),
            y: buf.readFloatLE(offset + 12),
          },
          dst: {
            x: buf.readFloatLE(offset + 0),
            y: buf.readFloatLE(offset + 4),
    ...
    meshs.push({
      id: i,
      type: buf
        .slice(offsetMeshs + i * 0x30 + 4, offsetMeshs + i * 0x30 + 8)
        .toString() as ISCMeshType,
      name: readASCII(buf, offsetMeshs + i * 0x30 + 0x20),
      textureId,
      isSpring,
      triangleList: chunk(
        range(ptSize).map((j) => buf.readInt32LE(ptOffset + j * 4)),
        3
  ...
  for (let i = 0; i < numIKs; i++) {
    ...
    if (type !== 0x54534349) throw new Error(`unknown ik type: ${type}`);
...
function load2(buf: Buffer, version: string): ISC2 {
  const name = readASCII(buf, buf.readUInt32LE(16));

  const atlasSizeAligned = buf.readUInt32LE(16 + 0 * 8 + 0);
  // const jsonSizeAligned = buf.readUInt32LE(16 + 0 * 8 + 4);
  const atlasSize = buf.readUInt32LE(16 + 1 * 8 + 0);
  const jsonSize = buf.readUInt32LE(16 + 1 * 8 + 4);

  const atlas = buf.slice(32, 32 + atlasSize).toString("utf-8");
  const json = buf
    .slice(32 + atlasSizeAligned, 32 + atlasSizeAligned + jsonSize)
    .toString("utf-8");

  return { type: 2, version, name, atlas, json: JSON.parse(json) };
}
